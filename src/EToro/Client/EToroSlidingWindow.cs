...
/// When used via <see cref="EToroKeyedRateLimiter"/>, one window is created per user key;
/// idle windows are evicted and disposed automatically, so total memory is bounded.
/// </remarks>
internal sealed class EToroSlidingWindow : IDisposable
{
    ...
    public EToroSlidingWindow(int permitLimit, TimeSpan window, TimeProvider timeProvider)
    {
