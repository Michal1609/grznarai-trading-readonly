...
internal sealed class QueryStringBuilder
{
    ...
    {
        _parts.Add($"{Escape(name)}={value.ToString().ToLowerInvariant()}");
        return this;
    }

    public QueryStringBuilder Add(string name, Enum value)
    {
        _parts.Add($"{Escape(name)}={Escape(value.ToApiString())}");
        return this;
    }
    ...
    public QueryStringBuilder AddCsv(string name, IEnumerable<string>? values)
    {
        ...
        var list = values.Select(value => value.Trim()).Where(value => value.Length > 0).ToList();
        if (list.Count > 0)
