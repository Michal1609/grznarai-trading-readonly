...
| `EstimatedNetDeposits` | **Estimate** inferred as `Equity - TotalReturn`. Not a real deposit ledger â€” withdrawals, bonuses, dividends, and other cash flows may not be reflected. |
...
Duplicate `PositionId` values across pages are deduplicated automatically â€” each closed trade is counted at most once.
