...
/// <summary>
/// Produkční / průzkumné testy Trading Real API.
/// Cíl: odhalit chování eToro nedokumentované nebo odlišné od dokumentace —
/// pageSize/page chování, filtry minDate, konzistence PnL dat, chování GetOrderAsync.
/// Rate limit: 60 req/min — zajišťuje RateLimitHandler automaticky.
///
/// Spuštění: dotnet test --filter "FullyQualifiedName~TradingRealProductionApiTests"
/// </summary>
[TestFixture]
...
public class TradingRealProductionApiTests
{
    ...
    private int    _defaultPageCount;  // kolik záznamů vrátí stránka 0 bez pageSize

    // OrderId z PnL (OrdersForOpen + Orders) — největší = nejnovější otevřená objednávka
    ...
    public async Task SetUp()
    {
        ...
        // Seed: největší OrderId z PnL — Positions, OrdersForOpen, Orders
        // Historie obsahuje jen uzavřené pozice — pro GetOrderAsync je PnL lepší zdroj
        ...
        Debug.WriteLine($"[SetUp] PnL OrdersForOpen={pnl.OrdersForOpen.Count} Orders={pnl.Orders.Count} " +
                        $"→ hasPnlOrders={_hasPnlOrders} pnlMaxOrderId={_pnlMaxOrderId}");
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // GetPortfolioAsync — trading/info/portfolio
    // Dokumentace: žádné query parametry, vrátí positions + credit
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetPortfolioAsync_Real_LeverageValuesAreSensible()
    {
        ...
        foreach (var pos in result.Positions)
        {
            ...
            if (!validLeverages.Contains(pos.Leverage))
                Debug.WriteLine($"  ⚠ Neznámý leverage={pos.Leverage} (PositionId={pos.PositionId}).");
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetPnlAsync — trading/info/real/pnl
    // Dokumentace: žádné query parametry, vrátí kompletní portfolio s PnL
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetPnlAsync_Real_UnrealizedPnL_ConsistencyWithPositions()
    {
        ...
        // Součet PnL z přímých pozic
        var directPnL = result.Positions.Sum(p => p.PnL);
        // Součet PnL z mirror pozic
        ...
        Debug.WriteLine($"  Součet přímých pozic: {directPnL:+0.00;-0.00} ({result.Positions.Count} pos)");
        Debug.WriteLine($"  Součet mirror pozic:  {mirrorPnL:+0.00;-0.00} ({result.MirrorPortfolios.Sum(m => m.Positions.Count)} pos)");
        Debug.WriteLine($"  Celkový součet:       {summedPnL:+0.00;-0.00}");

        var diff = Math.Abs(result.UnrealizedPnL - summedPnL);
        if (diff > 1m)
            Debug.WriteLine($"  ⚠ Rozdíl {diff:F4} USD mezi UnrealizedPnL a součtem pozic — možná rounding nebo časová neshoda.");

        // Tolerujeme malé rozdíly (FX konverzní rounding, časová prodleva)
    ...
    public async Task GetPnlAsync_Real_vs_GetPortfolioAsync_PositionsCountMatch()
    {
        ...
        if (pnlDirectCount != portfolioCount)
            Debug.WriteLine($"  ⚠ Počty se liší — GetPnlAsync a GetPortfolioAsync vrátily různý počet přímých pozic!");

        Assert.That(pnlDirectCount, Is.EqualTo(portfolioCount),
            "GetPnlAsync a GetPortfolioAsync by měly vracet stejný počet přímých pozic.");
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // GetOrderAsync — trading/info/real/orders/{orderId}
    // Dokumentace: orderId path param (int64), 404 pro neexistující
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetOrderAsync_Real_ValidOrderId_ReturnsConsistentData()
    {
        if (!_hasPnlOrders)
        {
            Assert.Ignore("PnL neobsahuje žádné otevřené/čekající objednávky → orderId nedostupný.");
            return;
        }
    ...
    public async Task GetOrderAsync_Real_StatusId_IsKnownValue()
    {
        ...
        // Zdroj: největší OrderId z PnL.OrdersForOpen + PnL.Orders (otevřené, ne uzavřené)
        var result = await _client.GetOrderAsync(EToroEnvironment.Real, _pnlMaxOrderId);

        Debug.WriteLine($"[Order] pnlMaxOrderId={_pnlMaxOrderId} → StatusId={result.StatusId} OrderType={result.OrderType}");
    ...
    public async Task GetOrderAsync_Real_AllPnlOrderIds_ReturnConsistentData()
    {
        if (!_hasPnlOrders)
        {
            Assert.Ignore("PnL neobsahuje žádné otevřené/čekající objednávky.");
            return;
        }

        // Načteme PnL znovu pro kompletní seznam orderIds (max 5 abychom šetřili rate limit)
        var pnl = await _client.GetPnlAsync(EToroEnvironment.Real);
        ...
        foreach (var orderId in orderIds)
        {
            ...
            Debug.WriteLine($"  OrderId={orderId} → StatusId={result.StatusId} InstrumentId={result.InstrumentId} " +
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetTradeHistoryAsync — trading/info/trade/history
    // Dokumentace: minDate (required), page (optional), pageSize (optional)
    // HLAVNÍ cíl: odhalit zda eToro respektuje pageSize
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetTradeHistoryAsync_PageSize_ActualCountMatchesRequested(int pageSize)
    {
        ...
        Debug.WriteLine($"[History] pageSize={pageSize} → returned={returned} defaultPageCount={_defaultPageCount}");

        if (returned > pageSize)
            Debug.WriteLine($"  ⚠ API vrátilo {returned} > pageSize={pageSize} — parametr ignorován!");
        else if (returned < pageSize && returned < _defaultPageCount)
            Debug.WriteLine($"  ⚠ API vrátilo {returned} < pageSize={pageSize} (defaultPageCount={_defaultPageCount}) — eToro bug?");
        else if (returned == _defaultPageCount && returned < pageSize)
            Debug.WriteLine($"  ℹ Vráceno {returned} = defaultPageCount={_defaultPageCount} ≤ pageSize={pageSize} — ok, méně dat než stránka.");

        Assert.That(returned, Is.LessThanOrEqualTo(pageSize),
            $"API vrátilo {returned} trades, přestože pageSize={pageSize} — parametr ignorován.");
    }

    // ─── pageNumber indexování ────────────────────────────────────────────────
    ...
    public async Task GetTradeHistoryAsync_PageNumber_IndexingBehavior()
    {
        if (!_hasHistory)
        {
            Assert.Ignore("Žádná obchodní historie.");
            return;
        }
        ...
        if (p0EqP1)
            Debug.WriteLine("  ⚠ page0 == page1 — eToro indexuje od 1 nebo stránkování nefunguje!");
        ...
        if (_defaultPageCount > ps)
            Assert.That(p0EqP1, Is.False,
                "page0 == page1 přestože existuje dost dat — stránkování nefunguje nebo indexování od 1.");
        else
            Assert.Pass($"Nedostatek dat pro ověření stránkování (defaultPageCount={_defaultPageCount} ≤ pageSize={ps}).");
    }
    ...
    public async Task GetTradeHistoryAsync_MinDate_FilterWorks(int daysBack)
    {
        ...
        Debug.WriteLine($"[History] minDate={minDate:yyyy-MM-dd} (daysBack={daysBack}) → returned={result.Trades.Count}");

        if (result.Trades.Count == 0)
        {
            Debug.WriteLine("  ℹ Žádné obchody v daném období.");
            Assert.Pass($"Žádné obchody od {minDate:yyyy-MM-dd}.");
            return;
        }

        // Ověření: closeTimestamp musí být >= minDate (eToro by měl filtrovat)
        var tooOld = result.Trades.Where(t => t.CloseTimestamp < minDate).ToList();
        if (tooOld.Count > 0)
        {
            Debug.WriteLine($"  ⚠ {tooOld.Count} obchodů má CloseTimestamp před minDate={minDate:yyyy-MM-dd}:");
            foreach (var t in tooOld.Take(3))
                Debug.WriteLine($"    PositionId={t.PositionId} CloseTimestamp={t.CloseTimestamp:yyyy-MM-dd}");
        }

        // Informativní — eToro může filtrovat dle openTimestamp nebo jiného pole
        Assert.That(tooOld.Count, Is.EqualTo(0),
            $"{tooOld.Count} obchodů má CloseTimestamp před minDate={minDate:yyyy-MM-dd} — filtr nefunguje?");
    }
    ...
    public async Task GetTradeHistoryAsync_ShortRange_vs_LongRange_ShortHasLessOrEqual()
    {
        ...
        if (shortResult.Trades.Count > longResult.Trades.Count)
            Debug.WriteLine("  ⚠ Kratší rozsah vrátil více obchodů než delší — minDate filtr ignorován?");

        Assert.That(shortResult.Trades.Count, Is.LessThanOrEqualTo(longResult.Trades.Count),
            "Kratší časový rozsah by měl vracet ≤ počet obchodů než delší rozsah.");
    }
    ...
    public async Task GetTradeHistoryAsync_ClosedTrades_DataQuality()
    {
        ...
        Debug.WriteLine($"[History] Kontrola kvality {result.Trades.Count} obchodů.");
        ...
        foreach (var t in result.Trades)
        {
            ...
            Assert.Multiple(() =>
            {
                ...
                Assert.That(t.InstrumentId, Is.GreaterThan(0),
                    $"InstrumentId musí být kladné (PositionId={t.PositionId}).");
                Assert.That(t.OpenRate, Is.GreaterThan(0),
                    $"OpenRate musí být kladný (PositionId={t.PositionId}).");
                Assert.That(t.CloseRate, Is.GreaterThan(0),
                    $"CloseRate musí být kladný (PositionId={t.PositionId}).");
                Assert.That(t.Investment, Is.GreaterThanOrEqualTo(0),
                    $"Investment nesmí být záporný (PositionId={t.PositionId}).");
                Assert.That(t.Units, Is.GreaterThan(0),
                    $"Units musí být kladný (PositionId={t.PositionId}).");
                Assert.That(t.CloseTimestamp, Is.GreaterThanOrEqualTo(t.OpenTimestamp),
                    $"CloseTimestamp musí být ≥ OpenTimestamp (PositionId={t.PositionId}).");
                Assert.That(t.Leverage, Is.GreaterThanOrEqualTo(1),
                    $"Leverage musí být ≥ 1 (PositionId={t.PositionId}).");
            });
        }
    }
    ...
    public async Task GetTradeHistoryAsync_Fees_AreNonNegative()
    {
        ...
        if (negFees.Count > 0)
        {
            Debug.WriteLine($"[History] ⚠ {negFees.Count} obchodů se zápornými fees:");
            foreach (var t in negFees.Take(5))
                Debug.WriteLine($"  PositionId={t.PositionId} Fees={t.Fees}");
        }

        Assert.That(negFees, Is.Empty, "Fees nesmí být záporné.");
    }
    ...
    public async Task GetTradeHistoryAsync_NoPageSize_vs_PageSize_DefaultBehavior()
    {
        ...
        Debug.WriteLine($"[History] Bez pageSize → {noSize.Trades.Count} trades");
        Debug.WriteLine($"[History] pageSize=20 → {size20.Trades.Count} trades");

        if (noSize.Trades.Count == size20.Trades.Count)
            Debug.WriteLine("  ℹ Počty se shodují — možná defaultní pageSize API je 20.");
        else if (noSize.Trades.Count > 20)
            Debug.WriteLine($"  ℹ Defaultní pageSize API > 20 (vráceno {noSize.Trades.Count}).");

        Assert.That(size20.Trades.Count, Is.LessThanOrEqualTo(20),
            $"pageSize=20 ale vráceno {size20.Trades.Count} — parametr ignorován.");
    }
    ...
    public void GetTradeHistoryAsync_NegativePage_ClientThrows()
    {
        Assert.ThrowsAsync<ArgumentOutOfRangeException>(
            ...
            "Client by měl odmítnout záporné page.");
    }
    ...
    public void GetTradeHistoryAsync_ZeroPageSize_ClientThrows()
    {
        Assert.ThrowsAsync<ArgumentOutOfRangeException>(
            ...
            "Client by měl odmítnout pageSize=0.");
    }
    ...
    public void GetTradeHistoryAsync_NegativePageSize_ClientThrows()
    {
        Assert.ThrowsAsync<ArgumentOutOfRangeException>(
            ...
            "Client by měl odmítnout záporný pageSize.");
    }
}
