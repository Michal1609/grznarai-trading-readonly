...
/// <summary>
/// Produkční / průzkumné testy User-Info API.
/// Cíl: odhalit chování eToro nedokumentované nebo odlišné od dokumentace —
/// pageSize/page chování (page od 1!), filtry, DailyGain converter, všechny periody.
/// Rate limit: 60 req/min — zajišťuje RateLimitHandler automaticky.
///
/// Spuštění: dotnet test --filter "FullyQualifiedName~UserInfoProductionApiTests"
/// </summary>
[TestFixture]
...
public class UserInfoProductionApiTests
{
    ...
    private string  _popularUsername = string.Empty;  // popular investor — bohatší data
    ...
    public async Task SetUp()
    {
        ...
        // Vlastní identita → RealCid → vlastní profil → username
        ...
        Assert.That(ownProfile.Users, Is.Not.Empty, "Vlastní profil nenalezen podle CID");
        _ownUsername = ownProfile.Users[0].Username;
        _ownGcid     = ownProfile.Users[0].Gcid;

        // Popular investor — bohatší data pro TradeInfo / LivePortfolio
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetUserProfilesAsync — user-info/people
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetUserProfiles_OwnUsername_ReturnsOwnProfile()
    {
        ...
        Assert.That(own, Is.Not.Null, $"Vlastní username '{_ownUsername}' nenalezen v odpovědi");
    ...
    public async Task GetUserProfiles_UsernameAndCid_Consistency()
    {
        ...
        Assert.That(byName.Users[0].Gcid,    Is.EqualTo(byCid.Users[0].Gcid),    "Gcid se liší");
        Assert.That(byName.Users[0].RealCid, Is.EqualTo(byCid.Users[0].RealCid), "RealCid se liší");
        Assert.That(byName.Users[0].Username, Is.EqualTo(byCid.Users[0].Username), "Username se liší");
    }
    ...
    public async Task GetUserProfiles_NonExistentUsername_ReturnsEmpty()
    {
        ...
        // Dokumentace říká: vrátí prázdný seznam, ne chybu
        Assert.That(result.Users, Is.Not.Null);
        if (result.Users.Count > 0)
            Debug.WriteLine($"⚠ Neexistující username vrátil {result.Users.Count} uživatelů");
        else
            Debug.WriteLine("[NonExistentUsername] Správně vrátil prázdný seznam");
    }
    ...
    {
        if (!_hasPopularUser)
            Assert.Ignore("Nenalezen popular investor — test přeskočen");
        ...
        if (result.Users.Count < 2)
            Debug.WriteLine($"⚠ Vráceno {result.Users.Count} místo 2 profilů");
        Assert.That(result.Users.Count, Is.GreaterThanOrEqualTo(1));
    }

    // ═══════════════════════════════════════════════════════════════════════════
    // SearchUsersAsync — user-info/people/search
    // Poznámka: Page začíná od 1 (ne od 0)!
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task SearchUsers_AllPeriods_ReturnResults(UserInfoPeriod period)
    {
        ...
        if (result.Items.Count > result.TotalItems)
            Debug.WriteLine($"⚠ Items.Count({result.Items.Count}) > TotalItems({result.TotalItems})");
    }
    ...
    public async Task SearchUsers_PageSize_ItemsCountRespectsLimit(int pageSize)
    {
        ...
        Assert.That(result.Items.Count, Is.LessThanOrEqualTo(pageSize),
            $"API vrátilo {result.Items.Count} > pageSize {pageSize}");
        if (result.Items.Count < pageSize && result.TotalItems > pageSize)
            Debug.WriteLine($"⚠ API vrátilo méně ({result.Items.Count}) než pageSize ({pageSize}), " +
                            $"ale totalItems={result.TotalItems} — možný bug eToro");
    }
    ...
    public async Task SearchUsers_Page1VsPage2_DifferentItems()
    {
        ...
        {
            Assert.Ignore("Méně než 10 výsledků — stránkování nelze testovat");
            return;
        }
        ...
        if (overlap > 0)
            Debug.WriteLine($"⚠ Stránky 1 a 2 sdílí {overlap} UserName — možná chyba stránkování");
        Assert.That(overlap, Is.EqualTo(0), "Page 1 a Page 2 by neměly sdílet záznamy");
    }
    ...
    public async Task SearchUsers_TotalItemsVsActual_Consistency()
    {
        ...
        if (result.TotalItems < result.Items.Count)
            Debug.WriteLine($"⚠ TotalItems({result.TotalItems}) < skutečný počet({result.Items.Count})");
    }

    [Test]
    public void SearchUsers_PageZero_ThrowsArgumentOutOfRange()
    {
        // Page=0 je neplatné — validace v klientovi (musí být >= 1)
    ...
    public async Task SearchUsers_ItemData_IsValid()
    {
        ...
        {
            Assert.Ignore("Žádné výsledky — data nelze validovat");
            return;
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetUserDailyGainAsync — user-info/people/{username}/daily-gain
    // Poznámka: type=Daily → IsDaily=true (pole entries), type=Period → IsPeriod=true (objekt s gain)
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetUserDailyGain_TypeDaily_ReturnsDailyEntries()
    {
        ...
        Assert.That(result.IsDaily, Is.True, "Typ Daily by měl vrátit IsDaily=true");
    ...
    public async Task GetUserDailyGain_TypePeriod_ReturnsPeriodGain()
    {
        ...
        Assert.That(result,          Is.Not.Null);
        Assert.That(result.IsPeriod, Is.True, "Typ Period by měl vrátit IsPeriod=true");
    ...
    public async Task GetUserDailyGain_Daily_TimestampsInRequestedRange()
    {
        ...
        {
            Debug.WriteLine("[DailyTimestamps] Žádné záznamy — skip");
            return;
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetUserGainAsync — user-info/people/{username}/gain
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetUserGain_MonthlyEntries_TimestampsOrdered()
    {
        ...
        {
            Debug.WriteLine("[UserGain Order] Méně než 2 monthly záznamy — skip");
            return;
        }
        ...
        for (int i = 1; i < result.Monthly.Count; i++)
        {
            if (result.Monthly[i].Timestamp < result.Monthly[i - 1].Timestamp)
                Debug.WriteLine($"⚠ Monthly záznamy nejsou vzestupně seřazeny " +
                                $"na indexech {i - 1} a {i}");
        }
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetUserLivePortfolioAsync — user-info/people/{username}/portfolio/live
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetUserLivePortfolio_PositionData_IsValid()
    {
        ...
        {
            Debug.WriteLine("[LivePortfolio positions] Žádné pozice — skip validace");
            return;
        }
    ...
    public async Task GetUserLivePortfolio_SocialTradeData_IsValid()
    {
        ...
        {
            Debug.WriteLine("[LivePortfolio socialTrades] Žádné social trades — skip validace");
            return;
        }
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // GetUserTradeInfoAsync — user-info/people/{username}/tradeinfo
    // Všechny periody musí vrátit validní data
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    public async Task GetUserTradeInfo_DataRanges_AreValid()
    {
        ...
        if (Math.Abs(leverageSum - 1m) > 0.05m && leverageSum > 0)
            Debug.WriteLine($"⚠ LeveragePct součet={leverageSum:F3} (očekáváno ~1.0)");
    }
